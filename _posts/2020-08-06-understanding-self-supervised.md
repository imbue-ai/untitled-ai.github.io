...
In the remainder of this article, we will limit our discussion to some of the most recent state of the art self-supervised algorithms for image data, though the principles apply to other forms of data as well. For more background, see DeepMind's [blog post](https://deepmind.com/blog/article/unsupervised-learning) , a [practical overview](https://www.fast.ai/2020/01/13/self_supervised/) from FastAI, or [this in-depth post](https://lilianweng.github.io/lil-log/2019/11/10/self-supervised-learning.html) from Lilian Weng on everything related to self-supervised learning and representation learning. [This github repo](https://github.com/HobbitLong/PyContrast/blob/master/AWESOME_CONTRASTIVE_LEARNING.md) contains a number of other relevant papers as well.

[^1]: *unsupervised* is sometimes used interchangeably with *self-supervised*, though [as Yann LeCun points out](https://www.facebook.com/722677142/posts/10155934004262143/), the term *unsupervised* is both loaded and confusing.
...
{% include image.html 
url="/assets/img/understanding_self_supervised/Screen_Shot_2020-07-15_at_4.40.22_PM.png" 
alt="MoCo diagram" 
...
{% include image.html 
url="/assets/img/understanding_self_supervised/Screen_Shot_2020-07-28_at_1.56.15_PM.png" 
alt="BYOL diagram" 
...
{% include image.html 
url="/assets/img/understanding_self_supervised/Screen_Shot_2020-07-28_at_11.01.07_AM.png" 
alt="Linear evaluation accuracy on STL10" 
caption="Linear evaluation accuracy on a validation set during early training of a ResNet-18 on STL10. When BYOL is trained without batch normalization in the MLP, the performance remains no better than a random baseline." 
class="med-image"
%}
...
{% include image.html 
url="/assets/img/understanding_self_supervised/Screen_Shot_2020-07-27_at_6.44.04_PM.png" 
alt="Contrastive loss diagram" 
...
{% include image.html 
url="/assets/img/understanding_self_supervised/Screen_Shot_2020-07-28_at_10.50.27_AM.png" 
alt="Cosine similarity between z and z'" 
caption="The average cosine similarity between the projections $z$ and $z^\prime$. The lower (blue) bar is the similarity between projections of the same image $x$, while the upper (red) bar is the similarity between projections of different images in the same minibatch. The high similarity of all representations for the no MLP normalization experiment and the layer normalization experiment indicates representation collapse." 
class="med-image"
%}
